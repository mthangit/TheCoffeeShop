...
--
-- Máy chủ: 127.0.0.1
-- Thời gian đã tạo: Th6 12, 2023 lúc 05:45 PM
...
-- Thủ tục
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_AddBill` (IN `p_billDate` DATE, IN `p_customerName` VARCHAR(50), IN `p_billPrice` INT, IN `p_customerPhone` VARCHAR(20), IN `p_employeeID` INT, IN `p_customerID` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_AddCategory` (IN `categoryName` VARCHAR(20))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_AddCustomer` (IN `p_customerName` VARCHAR(50), IN `p_customerDOB` DATE, IN `p_customerAddress` VARCHAR(50), IN `p_customerPhone` VARCHAR(20), IN `p_customerEmail` VARCHAR(50))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_AddDiscount` (IN `p_discountID` VARCHAR(20), IN `p_discountName` VARCHAR(50), IN `p_discountQuantity` INT, IN `p_discountValue` INT, IN `p_discountStartDay` DATE, IN `p_discountEndDay` DATE)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_ApplyDiscount` (IN `p_name` VARCHAR(255))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_Deletecategory` (IN `p_categoryID` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_GetBestSellingDrinksInTimeRange` (IN `startDate` DATE, IN `endDate` DATE)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_GetBillPriceByMonth` (IN `yearInput` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_GetBillPriceByMonthYear` (IN `inputMonth` INT, IN `inputYear` INT)   BEGIN
  DECLARE lastDay INT;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_GetSalesForDate` (IN `saleDate` DATE)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_ListEmployeesByStatus` (IN `statusInput` VARCHAR(30))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_UpdateCustomerPointsPLus` (IN `customerPhoneNumber` VARCHAR(50), IN `additionalPoints` INT)   BEGIN
    UPDATE customers
    SET customerPoint = customerPoint + additionalPoints
    WHERE customerPhone = customerPhoneNumber;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_UpdateDiscount` (IN `p_discountName` VARCHAR(50), IN `p_discountQuantity` INT, IN `p_discountValue` INT, IN `p_discountStartDay` DATE, IN `p_discountEndDay` DATE, IN `p_discountID` VARCHAR(50))   BEGIN
    UPDATE discount
    SET
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_UpdateFoodItem` (IN `p_itemID` INT, IN `p_newPrice` VARCHAR(50), IN `p_newCategoryName` VARCHAR(20), IN `p_newItemName` VARCHAR(40))   BEGIN
    -- START TRANSACTION;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_UpdateFoodItemCategoryID` (IN `categoryNameParam` VARCHAR(50))   BEGIN
    UPDATE food_item fi
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_UpdateItemId` (IN `itemNameParam` VARCHAR(50))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_UpdateRate` (IN `customer_phone` VARCHAR(255))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_ViewBillsByDate` (IN `search_date` VARCHAR(20))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_ViewTotalBillAmountByDate` (IN `search_date` VARCHAR(20), OUT `total_amount` INT)   BEGIN
...
(5, '2023-06-12', 'Thắng', 55000, NULL, 1014, NULL),
(6, '2023-06-12', 'Vũ Anh Khoa', 177000, '0983666421', 1014, 101),
(7, '2023-06-12', 'Ngô Hà Thanh', 317000, '0798695531', 1014, 107);
...
(5, 'Cà phê đen', 1, 30000, 101),
(5, 'Cà phê trân châu DÂU', 1, 25000, 506),
(6, 'Trà đen đào', 1, 62000, 121),
(6, 'Mì trộn trứng', 1, 35000, 137),
(6, 'Cà phê muối', 1, 25000, 500),
(7, 'Trà sữa xoài trân châu đen', 1, 70000, 114),
(7, 'Strawberry choco cookie smoothie', 1, 70000, 127);
...
--
DELIMITER $$
CREATE TRIGGER `tg_check_duplicate_category` AFTER INSERT ON `category` FOR EACH ROW BEGIN
    DECLARE count INT;
...
CREATE TABLE `customers` (
  ...
  `customerStartDate` date DEFAULT NULL,
...
(101, 'Vũ Anh Khoa', '1998-07-30', '0983666421', 'Tân Nguyên, Bình Dương', 'khoava3007@gmail.com', '2022-12-25', 'Gold', 50264),
(102, 'Vũ Ngọc Oanh', '1999-06-03', '0938888777', 'Quận 5, TPHCM', 'oanhvn0306@gmail.com', '2022-12-25', 'Gold', 546),
...
(107, 'Ngô Hà Thanh', '1989-11-06', '0798695531', 'Dĩ An, Bình Dương', 'thanhnh0611@gmail.co', '2022-12-26', 'Silver', 172),
...
(123, 'Bùi Thùy Linh', '2005-03-28', '0968142103', 'Quận 11, TPHCM', 'linhbt2803@gmail.com', '2022-12-28', 'Gold', 1431),
...
(138, 'tứ', '2023-06-02', NULL, 'thu duc', 'tu123@gmail.com', '2023-06-10', 'Member', 0);

--
-- Bẫy `customers`
--
DELIMITER $$
CREATE TRIGGER `tg_check_duplicate_customerPhone` AFTER INSERT ON `customers` FOR EACH ROW BEGIN
    DECLARE count INT;
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `tg_check_start_date` AFTER INSERT ON `customers` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `tg_increment_customerID` BEFORE INSERT ON `customers` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `tg_set_default_values` BEFORE INSERT ON `customers` FOR EACH ROW BEGIN
...
('D07', 'SN7', 4, 5, '2023-06-04', '2023-08-04'),
...
--
DELIMITER $$
CREATE TRIGGER `tg_check_discount_dates` BEFORE INSERT ON `discount` FOR EACH ROW BEGIN
    IF NEW.discountStartDay >= NEW.discountEndDay THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Ngày bắt đầu phải nhỏ hơn ngày kết thúc';
    END IF;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `tg_check_discount_dates_update` BEFORE UPDATE ON `discount` FOR EACH ROW BEGIN
    IF NEW.discountStartDay >= NEW.discountEndDay THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Ngày bắt đầu phải nhỏ hơn ngày kết thúc';
    END IF;
END
...
(350, 'Cà phê LATTE', 25000, 'Cà phê', 1),
(400, 'Cà phê trứng', 25000, 'Cà phê', 1),
(450, 'Cà phê american', 25000, 'Cà phê', 1),
(500, 'Cà phê muối', 25000, 'Cà phê', 1),
(503, 'Cà phê trân châu', 25000, 'Cà phê', 1),
(506, 'Cà phê trân châu DÂU', 25000, 'Cà phê', 1),
(508, 'Cà phê COLD', 15000, 'Cà phê', 1);

--
-- Bẫy `food_item`
--
DELIMITER $$
CREATE TRIGGER `tg_check_item_price_INSERT` AFTER UPDATE ON `food_item` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `tg_check_item_price_UPDATE` AFTER UPDATE ON `food_item` FOR EACH ROW BEGIN
    IF NEW.itemPrice <= 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'itemPrice must be greater than 0';
    END IF;
END
...
ALTER TABLE `bill_detail`
  ...
  ADD CONSTRAINT `bill_detail_ibfk_2` FOREIGN KEY (`itemID`) REFERENCES `food_item` (`itemID`);
